...
function precision2(predictions, labels)
    tp = 0
    fp = 0
    for i in 1:length(labels)
      if labels[i] == 1
...
function cross_validation(k, learn_method, data, labels, evaluation_metric)
  n = size(data,1)
  if n < k
  ...
  if (fold_indices[end][end][1] < n)
      vcat(fold_indices[end][end][1],(fold_indices[end][end][1]+1):n)
  else
      fold_indices[end]
  end
  ...
  for fold = fold_indices
      ...
      training_dataset = data[training_indices,:]
